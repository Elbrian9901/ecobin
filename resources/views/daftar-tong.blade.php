    ...
    @else
        <div class="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-4">
            ...
            <div class="bg-white rounded-2xl shadow-sm border border-[#e5ede5] p-5 hover:shadow-md transition-shadow">
                ...
                @if($tong->no_whatsapp)
                <div class="flex items-center justify-between gap-1.5 text-xs text-[#7a9a85] mb-4">
                    ...
                    <a href="https://wa.me/{{ preg_replace('/\D/', '', preg_replace('/^0/', '62', $tong->no_whatsapp)) }}"
                       target="_blank"
                       class="inline-flex items-center gap-1 text-[11px] font-semibold text-emerald-700 bg-emerald-50 px-2.5 py-1 rounded-full hover:bg-emerald-100 transition">
                        Chat WhatsApp
                    </a>
                </div>
                @endif

                {{-- Progress bar status (kapasitas liter disembunyikan dari tampilan) --}}
                @php
                    $statusLabel = $tong->persen >= 90 ? 'Penuh' : ($tong->persen >= 70 ? 'Hampir Penuh' : 'Normal');
                    $statusTextColor = $tong->persen >= 90 ? 'text-red-500' : ($tong->persen >= 70 ? 'text-amber-500' : 'text-[#22a846]');
                @endphp
                <div class="mb-1 flex justify-between items-center">
                    <span class="text-xs font-semibold {{ $statusTextColor }}">{{ $statusLabel }}</span>
    ...
    <div class="bg-white rounded-2xl shadow-2xl w-full max-w-md p-6 relative">
        ...
        <form method="POST" action="{{ route('tambah-tong') }}" class="space-y-4">
            ...
            <div>
                <label class="block text-sm font-semibold text-[#1a3d28] mb-1.5">
                    Nomor WhatsApp Pengurus
                    <span class="text-[#a8bfb0] font-normal">(contoh: 081234567890)</span>
                </label>
                <div class="relative">
                    ...
                    <input type="text" name="no_whatsapp"
                           ...
                           pattern="^(08[0-9]{9,11}|\+62[0-9]{9,11})$"
                           title="Format harus diawali 08 atau +62, dengan total 11-13 digit"
                ...
                <p class="text-xs text-[#a8bfb0] mt-1">Format: diawali 08 atau +62, total 11-13 digit, contoh: 081234567890</p>
                @error('no_whatsapp')
                    <p class="text-xs text-red-600 font-semibold mt-1">{{ $message }}</p>
                @enderror
            </div>

            {{-- Kapasitas disembunyikan dari admin: dikirim otomatis default 60L untuk keperluan kalkulasi persentase --}}
            <input type="hidden" name="kapasitas" value="60">

            @if($errors->any())
                <div class="bg-red-50 border border-red-200 text-red-700 rounded-xl px-4 py-2.5 text-xs space-y-1">
